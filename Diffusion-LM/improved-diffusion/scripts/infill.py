...
sys.path.insert(0, '/home/ec2-user/Diffusion-LM/transformers/examples/pytorch/language-modeling')
...
def main():
    ...
    if args.modality in ['synth', 'pos']:
        ...
        if args.eval_task_ == 'infill':
            ...
            if args.eval_task_ == 'control_attribute':
                model_control = Classifier_GPT2.from_pretrained('/home/ec2-user/Diffusion-LM/classifier_models/e2e-back_e=6_b=10_m=gpt2_wikitext-103-raw-v1_101_wp_full_multi_sqrt_16').cuda()

                control_label_lst = []
                with open('/home/ec2-user/Diffusion-LM/datasets/control_target/target_attribute.json', 'r') as controlf:
                ...
                for label_class in control_label_lst:
                    ...
                    control_constraints.append((langevin_fn_selected, label_class, label_ids))
        ...
        for (encoded_seq, control_helper) in zip(encoded_partial_seq, partial_seq) :
            ...
            while len(all_images) * args.batch_size < args.num_samples:
                ...
                if args.eval_task_.startswith('control'):
                    langevin_fn_selected, label_class_attributes, label_ids = control_helper
                    ...
                    noise = th.randn(sample_shape, device=model_control.device)
                    if args.init_samples > 0:
                        for i in range(args.batch_size):
                            sample_selected = None
                            for _ in range(args.init_samples):
                                label_ids2 = label_ids.expand(1, -1).cuda()
                                sample = th.randn((1, sample_shape[1], sample_shape[2]), device=model_control.device)
                                tgt_embs = model3(label_ids2[:, sample.size(1):])
                                label_ids2[:, :65] = -100
                                input_embs_param = th.nn.Parameter(sample)
                                input_embs = th.cat([input_embs_param, tgt_embs], dim=1)

                                with th.no_grad():
                                    model_out = model_control(input_embs=input_embs, labels=label_ids2, t=200)

                                if sample_selected is None:
                                    min_loss = model_out.loss
                                    sample_selected = sample
                                elif model_out.loss < min_loss:
                                    min_loss = model_out.loss
                                    sample_selected = sample 

                            noise[i, :, :] = sample_selected

                    label_ids2 = label_ids.expand(args.batch_size, -1).cuda()
                    tgt_embs = model3(label_ids2[:, noise.size(1):])
                    label_ids2[:, :65] = -100
                    input_embs = th.cat([noise, tgt_embs], dim=1)
                    with th.no_grad():
                        loss = model_control(input_embs=input_embs, labels=label_ids2, t=200).loss
                    losses[-1] += loss
                    ...
                    for sample in loop_func_(
                        ...
                        final = sample["sample"]
                        label_ids2 = label_ids.expand(args.batch_size, -1).cuda()
                        tgt_embs = model3(label_ids2[:, final.size(1):])
                        label_ids2[:, :65] = -100
                        input_embs = th.cat([final, tgt_embs], dim=1)
                        with th.no_grad():
                            loss = model_control(input_embs=input_embs, labels=label_ids2, t=time_step).loss
                        losses[time_step] += loss.cpu().item()
                        time_step -= 1
    ...
    if args.verbose == 'pipe':
        ...
        out_path_pipe = os.path.join(args.out_dir, f"{model_base_name}.infill_{args.eval_task_}_{args.notes}_{args.init_samples}.json")
        ...
        losses /= len(partial_seq)
        out_path_loss = os.path.join(args.out_dir, f"{model_base_name}.infill_{args.eval_task_}_{args.notes}_{args.init_samples}.npy")
        np.save(out_path_loss, losses)
        print(f'written the classifier losses to {out_path_loss}')
        ...
        out_path2 = os.path.join(args.out_dir, f"{model_base_name}.infill_{args.eval_task_}_{shape_str}_{args.notes}_{args.init_samples}.txt")
        ...
        ##############
        out_path2 = os.path.join(args.out_dir, f"{model_base_name}.infill_{args.eval_task_}_{shape_str}_{args.notes}_{args.init_samples}.json")
        fout = open(out_path2, 'w')
        for (xx) in zip(word_lst):
...
def create_argparser():
    defaults = dict(
        ...
        out_dir="./out_gen", init_samples=10,
